...
static inline void insert_to_tree(void *root, void *newnode, size_t size)
{
    size_t size_root;
    ...
    // find the node that satisfy binary search tree
    while (1)
    {
        size_root = GET_SIZE(HDPR(root));
        right_val = GET_RIGHT_VAL(root);
        left_val = GET_LEFT_VAL(root);

        place = size > size_root ? 1 : 0;
        temp_val = size > size_root ? right_val : left_val;
