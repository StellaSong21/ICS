 ...
 * The reason for this design is to optimize
...
//        extendsize = newsize; //58 points
        if ((ptr = mem_sbrk(extendsize)) == (void *) -1)
            return NULL;
        PUT(ptr + extendsize - WSIZE, 1);
        PUT(HDPR(ptr), PACK(extendsize, 0));
        PUT(FTPR(ptr), PACK(extendsize, 0));
        freelist_add(ptr);
    }
