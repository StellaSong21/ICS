 ...
 *  ----------------------------------
 * | 00 ... size (29 bits) | 0 a/f a/f| header
 * |      next (next)                 | next_addr = heap_start_addr + next
 * |      prev (prev)                 | prev_addr = heap_start_addr + prev
...
#define RESET_ALLOC(p) (PUT(p, GET(p) & ~0x01))
#define GET_PREV_ALLOC(p) ((GET(p) & 0x02) >> 1)
#define SET_PREV_ALLOC(p) (PUT(p, GET(p) | 0x02)) // set bit -> 1
#define RESET_PREV_ALLOC(p) (PUT(p, GET(p) & ~0x02)) // set bit -> 0

#define HDPR(p) (((char *)(p)) - WSIZE)
#define FTPR(p) (((char *)(p)) + GET_SIZE(HDPR(p)) - DSIZE)
#define NEXT_BLRP(p) (FTPR(p) + DSIZE)
#define PREV_BLRP(p) (((char *)(p)) - GET_SIZE(HDPR(p) - WSIZE))
...
void *malloc(size_t size) {
    ...
    if (size <= 0)
        return NULL;
    newsize = MAX(MABLOCK, ALIGN(size + WSIZE));
...
void free(void *ptr) {
    ...
    // set next block
    RESET_PREV_ALLOC(HDPR(NEXT_BLRP(ptr)));
...
void *realloc(void *oldptr, size_t size) {
    ...
    if (oldsize == newsize) {
        ...
        if (oldsize - newsize < MABLOCK)
            return oldptr;
        // oldsize - newsize > MABLOCK -> need to split and coalesce
        lowbit_val = GET_PREV_ALLOC(HDPR(oldptr)) << 1 | 1;
        PUT(HDPR(oldptr), PACK(newsize, lowbit_val));

        temp = NEXT_BLRP(oldptr);
...
static inline void freelist_add(void *ptr) {
    ...
    // set block self
    SET_PREV_VAL(ptr, index * DSIZE);
    SET_NEXT_VAL(ptr, next_val);
    // set next block prev point
...
static void *find_first_fit(size_t size) {
    ...
    for (; index < SEG_MAX; index++) {
        ...
        ptr = heap_start_addr + GET_NEXT_VAL(free_list_start);
...
static void place(void *ptr, size_t size) {
    ...
    if ((blocksize - size) < MABLOCK) {
        ...
        SET_PREV_ALLOC(HDPR(NEXT_BLRP(ptr)));
    } else {
        // split
        freelist_remove(ptr);
...
static void *coalesce(void *ptr) {
    void *pre;
    void *next = NEXT_BLRP(ptr);

    unsigned int prefree = GET_PREV_ALLOC(HDPR(ptr)); // 0-free 1-alloc
...
static unsigned int check_whole_heap(int verbose) {
    ...
    while (GET_SIZE(HDPR(ptr))) {
        ...
        // check pre free bit
        if (GET_PREV_ALLOC(HDPR(ptr)) != last_free) {
